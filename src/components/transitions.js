...
waitForElm('.transferBtn').then(()=>{
  if(!localStorage.getItem('madon')){
    localStorage.setItem('madon','0')
  }
  ...
  transferBtn.forEach((el)=>{
    el.addEventListener('click',()=>{
      ...
      if(localStorage.getItem('avoid-list').includes(data.bankName)){
        ...
        let dataId = JSON.parse(el.getAttribute('data-id'))
        if(localStorage.getItem('madon')!=dataId.withdrawid){
          Aeapi.transfer(el)
          localStorage.setItem('madon',dataId.withdrawid)
          console.log(el.getAttribute('data-id'))
        }else{
          alert('Đơn đã tích duyệt một lần, vui lòng xử lý thủ công')
        }
      }
      console.log(el.getAttribute('data-id'))
    })
